...
function audit(overrides: Record<string, unknown> = {}) {
  return {
    ...
    additionalDetails: [
      { key: "AuthenticationMethod", value: "Microsoft Authenticator" },
    ],
...
test("normalizes supported registrations from the target user, not the actor", () => {
  const registration = normalizeAudit(
    audit({
...
test("rejects actor-only, malformed, and non-registration audit records", () => {
  assert.equal(
    normalizeAudit(
      audit({ targetResources: [], initiatedBy: { user: { id: USER_ID } } }),
    ),
    undefined,
  );
  assert.equal(
  ...
  assert.equal(
    normalizeAudit(audit({ activityDateTime: "not a date" })),
    undefined,
  );
  assert.equal(
    normalizeAudit(audit({ activityDisplayName: "Update user" })),
    undefined,
  );
});

test("uses the device-bound passkey event and excludes its generic paired event", () => {
  const passkey = normalizeAudit(
    audit({
      activityDisplayName: "Add Passkey (device-bound)",
      additionalDetails: [],
    }),
  );
  const genericPair = normalizeAudit(
    audit({
      activityDisplayName: "User registered security info",
...
test("never uses unrecognized authentication-detail text as notification content", () => {
  const registration = normalizeAudit(
    audit({
      additionalDetails: [
        { key: "AuthenticationMethod", value: "+1 (555) 0100" },
      ],
    }),
  );
...
test("validates delivery configuration and its pilot safety boundary", () => {
  ...
  assert.throws(
    () => config({ EMAIL_SENDER_USER_ID: "not-a-uuid" }),
    /EMAIL_SENDER_USER_ID/,
  );
  assert.throws(
    () => config({ AUDIT_OVERLAP_MINUTES: "1441" }),
  ...
  const allUsers = config({
    PILOT_USER_IDS: undefined,
    ALL_USERS: "true",
    ADMIN_CHANNELS: "teams",
...
test("plans in-scope user and admin deliveries without duplicate recipient-channel sends", () => {
  ...
  assert.deepEqual(deliveries, [
    {
      key: `${registration.id}:email:${USER_ID}`,
      audience: "user",
      channel: "email",
      recipientId: USER_ID,
    },
    {
      key: `${registration.id}:teams:${USER_ID}`,
      audience: "user",
      channel: "teams",
      recipientId: USER_ID,
    },
    {
      key: `${registration.id}:email:${ADMIN_ID}`,
      audience: "admin",
      channel: "email",
      recipientId: ADMIN_ID,
    },
    {
      key: `${registration.id}:teams:${ADMIN_ID}`,
      audience: "admin",
      channel: "teams",
      recipientId: ADMIN_ID,
    },
  ...
  assert.deepEqual(
    planDeliveries(registration, config({ PILOT_USER_IDS: ADMIN_ID })),
    [],
  );
});
...
test("renders escaped user and admin notifications with a fixed security-info link", () => {
  ...
  assert.equal(
    (notification.card.body as Array<Record<string, unknown>>)[1].type,
    "FactSet",
  );

  const userNotification = renderNotification(
    registration,
    "user",
    "contact helpdesk",
  );
