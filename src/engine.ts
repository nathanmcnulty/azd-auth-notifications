import { normalizeAudit, planDeliveries, type Config, type Registration, type Delivery } from './core.js';
export interface Job extends Delivery { registration: Registration; status: string; etag?: string }
...
export interface AuditReader { audits(from:string,to:string):AsyncIterable<unknown[]> }
export interface DispatchOutcome { key:string; status:'accepted'|'review'|'suppressed'; code?:string }
export async function collect(config:Config,state:State,graph:AuditReader,now=new Date()):Promise<number> {
  if (!config.enabled) return 0;
  const end=now.toISOString();
  const checkpoint=await state.checkpoint();
  if (!checkpoint) { await state.saveCheckpoint({start:end,cursor:end}); return 0; }
  const from=new Date(Math.max(Date.parse(checkpoint.start),Date.parse(checkpoint.cursor)-config.overlapMinutes*60000)).toISOString();
  let count=0;
  for await (const page of graph.audits(from,end)) {
    for (const raw of page) {
      const event=normalizeAudit(raw);
      if (!event || event.occurredAt<checkpoint.start || Date.parse(event.occurredAt)>now.getTime()) continue;
      for (const delivery of planDeliveries(event,config)) {
        await state.add({...delivery,registration:event,status:'pending'}); count++;
      }
    }
  }
...
export async function dispatch(
  ...
  for await (const job of state.pending()) {
    ...
    if (!planDeliveries(job.registration,config).some(d=>d.key===job.key)) {
      if (!await state.claim(job)) continue;
      await state.finish(job,'suppressed','DeliveryNoLongerEligible');
      onOutcome?.({key:job.key,status:'suppressed',code:'DeliveryNoLongerEligible'});
      continue;
    }
    if (!await state.claim(job)) continue;
    try { await send(job); }
      ...
      const code=error instanceof Error && /^[A-Za-z0-9_:-]{1,80}$/.test(error.message)?error.message:'DeliveryOutcomeUnknown';
