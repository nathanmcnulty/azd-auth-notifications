...
var storageName = take('stan${namePrefix}${resourceToken}', 24)
...
resource blobRole 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  ...
  scope: storage
  properties: { principalId: identity.properties.principalId, principalType: 'ServicePrincipal', roleDefinitionId: subscriptionResourceId('Microsoft.Authorization/roleDefinitions', blobDataOwner) }
}
resource queueRole 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  ...
  scope: storage
  properties: { principalId: identity.properties.principalId, principalType: 'ServicePrincipal', roleDefinitionId: subscriptionResourceId('Microsoft.Authorization/roleDefinitions', queueDataContributor) }
}
resource tableRole 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(storage.id, identity.id, tableDataContributor)
  scope: storage
  properties: { principalId: identity.properties.principalId, principalType: 'ServicePrincipal', roleDefinitionId: subscriptionResourceId('Microsoft.Authorization/roleDefinitions', tableDataContributor) }
}
...
resource insightsRole 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  ...
  scope: insights
  properties: { principalId: identity.properties.principalId, principalType: 'ServicePrincipal', roleDefinitionId: subscriptionResourceId('Microsoft.Authorization/roleDefinitions', monitoringMetricsPublisher) }
}
...
resource functionApp 'Microsoft.Web/sites@2024-04-01' = {
  ...
  properties: {
    ...
    functionAppConfig: {
      deployment: { storage: { type: 'blobContainer', value: '${storage.properties.primaryEndpoints.blob}${deploymentContainer.name}', authentication: { type: 'UserAssignedIdentity', userAssignedIdentityResourceId: identity.id } } }
    ...
    siteConfig: {
      ...
      appSettings: [
        ...
        { name: 'USER_CHANNELS', value: userChannels }
        { name: 'ADMIN_CHANNELS', value: adminChannels }
        { name: 'ADMIN_USER_IDS', value: adminUserIds }
        { name: 'PILOT_USER_IDS', value: pilotUserIds }
        { name: 'ALL_USERS', value: string(allUsers) }
        { name: 'COLLECTION_ENABLED', value: string(collectionEnabled) }
...
resource ftpPolicy 'Microsoft.Web/sites/basicPublishingCredentialsPolicies@2024-04-01' = { parent: functionApp, name: 'ftp', properties: { allow: false } }
resource scmPolicy 'Microsoft.Web/sites/basicPublishingCredentialsPolicies@2024-04-01' = { parent: functionApp, name: 'scm', properties: { allow: false } }
resource diagnostics 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = {
  name: 'send-to-log-analytics'
  scope: functionApp
  properties: { workspaceId: workspace.id, logs: [{ categoryGroup: 'allLogs', enabled: true }], metrics: [{ category: 'AllMetrics', enabled: true }] }
}
resource bot 'Microsoft.BotService/botServices@2022-09-15' = if (teamsBotEnabled) {
  ...
  kind: 'azurebot'
  sku: { name: 'F0' }
  properties: { displayName: 'Authentication notifications', endpoint: 'https://${functionApp.properties.defaultHostName}/api/messages', msaAppId: identity.properties.clientId, msaAppMSIResourceId: identity.id, msaAppTenantId: tenantId, msaAppType: 'UserAssignedMSI' }
}
